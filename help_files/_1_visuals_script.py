...
# Plot using the dynamically found keys
def plot_loss_2(history):
    loss_key = [key for key in history.history.keys() if 'loss' in key and 'val' not in key][0]
    val_loss_key = [key for key in history.history.keys() if 'loss' in key and 'val' in key][0]
...
def plot_precision(history):
    ...
    val_precision_key = [key for key in history.history.keys() if 'val_precision' in key][0]
...
def plot_recall(history):
    ...
    val_recall_key = [key for key in history.history.keys() if 'val_recall' in key][0]
